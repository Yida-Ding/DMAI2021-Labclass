...
class SnakeNN:
    def __init__(self, initial_games = 10000, test_games = 1000, goal_steps = 30, lr = 1e-2, filename = 'snake_nn_2.tflearn'):
    ...
    def model(self):
        '''Build the newwork that could be connected by the model'''
        ???
    ...
    def test_model(self, model):
        ...
        for _ in range(self.test_games):
            ...
            for _ in range(self.goal_steps):
'''
Generate the current action 'game_action' with the prediction from the model produced
'''             ???
                ...
                game_action = ???
